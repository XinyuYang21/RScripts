...
```{r cars}
  ...
  for (i in types) {
    ...
    if (i %in% c("COADREAD","STAD","UCEC"))  {
      ...
      fabcolors <- list(Subtype = Subtype_colors,MSI_status=MSI_col,Escape=Escape_colors,Tcell_score=Tscore,POLE.altered=POLE_col,BLS.altered=BLS_col) 
      bottom_feature = "Neoantigen_mutation"
    } else {
      oncoplot.feature=c("Subtype","MSI_status","POLE.altered","BLS.altered")
      fabcolors <- list(Subtype = Subtype_colors,MSI_status=MSI_col,POLE.altered=POLE_col,BLS.altered=BLS_col) 
      bottom_feature = NULL
    }
    
    Coocrrence_network_plot_MSI(types=i, manSelect= manSelect,gistic=gistic_type,ks_path=ks_path,ccf_path=ccf_path,output=output,top_gene=top_gene,p_value=p_value,ccf_type=ccf_type,oncoPath=oncoPath,oncoplot.feature=oncoplot.feature,oncoplot.feature.col=fabcolors,
