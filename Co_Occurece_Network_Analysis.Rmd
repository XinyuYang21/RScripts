---
title: "Co_occurrence_network.rmd"
output: html_document
editor_options:
  chunk_output_type: console
---

```{r setup, include=FALSE}
  library(TCGAmutations)
  library(tidyverse)
  library(maftools)
  library(ComplexHeatmap)
  library(TCGAmutations)
  library(tidyverse)
  library(magrittr)
  library(maftools)
  library(ComplexHeatmap)
  library(gridExtra)
  library(circlize)
  library(ggsci)
  library(RColorBrewer)
  library(patchwork)
  library(visNetwork)
  library(igraph)
  library(dplyr)
  library(cooccur)
  library(igraph)
  library(qgraph)
  library(scales) # rescale()
  library(Hmisc)  # %nin%
  library(ggraph)
  library(tidyverse)
  library(igraph)
  library(ggraph)
  library(graphlayouts)
  library(ggforce)
  library(scatterpie)
  library(tidygraph) # activate
  library(ggforce)
  library(concaveman)
  library(patchwork)

  folder_path = "C:/Users/PC/Desktop/Xinyu/Evolution/GlasgowCancerEvolution/"

  source(paste0(folder_path,'Figures and Data/RScripts/oncomatrix.R'))
  source(paste0(folder_path,'Figures and Data/RScripts/getInteractions.R'))
  source(paste0(folder_path,'Figures and Data/RScripts/occurrence_network_plot.R'))
```

## Run 

```{r cars}

  man = read.csv(paste0(folder_path,"Figures and Data/ESigs_integrated_measure_4146_20210501.csv"))
  ks = read.csv(paste0(folder_path,"Figures and Data/CCF_transition/gene_ks_fisher_table_4000_20210502.csv"))
  oncoPath <-  read.csv(paste0(folder_path,"Figures and Data/Oncogenic_pathways.csv"))
  
  types= unique(man$cancertype)
  types = c("COADREAD")
 
  dir_all = dir(paste0(folder_path,"Figures and Data/CCF/"))
  i = types
  for (i in types) {
    
    if (nchar(i)==8) typesMaf = c(substr(i,1,4),substr(i,5,8)) else typesMaf = i
   
    sample_filter <- man %>% 
      dplyr::select(c("samplename", "subtype", "cancertype.x")) %>% 
      filter(subtype %in% c("ESig4"),cancertype.x %in% typesMaf)
      #filter(subtype %in% c("ESig3"),cancertype.x %in% typesMaf)
    
    gene_filter <- ks %>%
      filter(cancertype %in% paste0(sort(i),collapse  = "")) %>% 
      filter(significant_ks == 1)
    
    # get ccf file for filtered samples
    ccf_dir = dir_all[sapply(typesMaf,function(x) which(grepl(x,dir_all)))]
    
    for (j in 1:length(ccf_dir) ) {
      if (j==1) ccf_all= get(load(paste0(folder_path,"Figures and Data/CCF/",ccf_dir[1]))) else 
        ccf_all = rbind(ccf_all,get(load(paste0(folder_path,"Figures and Data/CCF/",ccf_dir[j]))))
    }
    ccf_filter = ccf_all %>% 
      mutate(samplename = substr(Matched_Norm_Sample_Barcode,1,12)) %>%
      filter(samplename %in% sample_filter$samplename)
    
    
    # Set top genes for co-occurrence analsysis, ccf types as the x axis and maximum p value filtered as network
    top_gene=30;ccf_type="median_ccf";p_value=0.05
    # top_gene=50;ccf_type="median_ccf";p_value=0.05
    # top_gene=30;ccf_type="median_ccf";p_value=0.01
    # top_gene=50;ccf_type="mean_ccf";p_value=0.01
    # top_gene=50;ccf_type="mean_ccf";p_value=0.05
    # top_gene=30;ccf_type="mean_ccf";p_value=0.05
    # top_gene=30;ccf_type="mean_ccf";p_value=0.01
    
    output_folder = paste0(folder_path,"Figures and Data/Concurrence_network_",Sys.Date(),"/")
    if (!dir.exists(output_folder)) dir.create(output_folder)
    output=paste0(output_folder,i,"_",top_gene,"genes_",ccf_type,"_",nrow(sample_filter),"_samples_",p_value,".pdf")
    
    Coocrrence_network_plot(types=i,ksByCancer=gene_filter,manSelect=sample_filter,output=output,top_gene=top_gene,p_value=p_value,ccf_type=ccf_type,ccf_all= ccf_filter,oncoPath=oncoPath)
    
  }

  # Trevor.Graham.et.al <- read.delim("C:/Users/PC/Desktop/Xinyu/Evolution/GlasgowCancerEvolution/Archive/Data/TCGA/Trevor Graham et al.txt")
  # colnames(  Trevor.Graham.et.al)[1] = "samplename"

  
```

