...
getInteractions <- function(maf, top = 25, genes = NULL, pvalue = c(0.05, 0.01), returnAll = TRUE,
  ...
  sigPairsTbl = data.table::rbindlist(
    lapply(X = seq_along(1:nrow(sigPairs)), function(i) {
      ...
      log10pval=interactions[x[1], x[2]]
      fest = oddsRatio[x[1], x[2]]
      d = data.table::data.table(gene1 = g1,
                                 gene2 = g2,
                                 pValue = pval, log10pval=log10pval, oddsRatio = fest)
      d = cbind(d, combn)
      d
    }), fill = TRUE)
  ...
  if(nrow(interactions) >= 5){
    ...
    if(showSigSymbols){
      ...
      points(w, pch=pvSymbols[2], col="white", cex = sigSymbolsSize)
      #w = arrayInd(which(10^-abs(interactions) < max(pvalue)), rep(m,2))
      w = arrayInd(which((10^-abs(interactions) < max(pvalue)) & (10^-abs(interactions) > min(pvalue))), rep(m,2))
      points(w, pch=pvSymbols[1], col="white", cex = sigSymbolsSize)
    }
  ...
  res <- list(pairs=sigPairsTblSig, interactions=interactions)
  return(res)
}
